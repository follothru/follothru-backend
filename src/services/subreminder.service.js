module.exports = (() => {
    ...
    function getOccurrence(startDate, endDate, repeats){
        ...
        const occurrence = repeats.map(repeat => {
            ...
            if(repeat.match(regexes.daily)) {
                ...
                return [ ...new Array(numOfDays).keys() ].map(index => new Date(startDate.getTime() + index * ONEDAYINMILL));
            }else if (repeat.match(regexes.weekly)) {
                const numOfDays = calNumOfDays(startDate, endDate, ONEDAYINMILL * 7);
                return [ ...new Array(numOfDays).keys() ].map(index => new Date(startDate.getTime() + index * ONEDAYINMILL * 7));
    ...
    return { findAllSubreminders, getSubremindersForReminder, createSubreminders };
