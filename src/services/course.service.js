module.exports = (() => {
    ...
    function findAllCourses() {
      return CourseModel.find().populate('instructors').then(courses =>
        courses.map(course => {
          const id = course._id;
      ...
      return new Promise((resolve, reject) => {
        try {
          ValidationUtils.notNullOrEmpty(name);
            const instructors = instructorIds.map(id => new mongoose.Types.ObjectId(id));
            const _id = new mongoose.Types.ObjectId();
            const newCourse = new CourseModel({ _id, name, description, instructors });
            newCourse
              .save()
              .then(result => resolve({ id: result._id })).catch(reject);
        } catch (err) {
          reject(err);
        }
      });
